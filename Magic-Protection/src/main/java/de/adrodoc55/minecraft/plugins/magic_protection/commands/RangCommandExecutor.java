...
public class RangCommandExecutor implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		if (player == null) {
			String message = String.format(ChatColor.RED
		...
		if ("get".equals(action)) {
			...
			String message = String.format("%s hat den Rang %d", playerName,
					rang);
			sender.sendMessage(message);
			return true;
			...
			try {
				...
				String message = String.format(ChatColor.RED
						+ "'%s' ist keine Zahl", args[2]);
				sender.sendMessage(message);
				return false;
			}
			PlayerManager.setPlayerRanking(player, rank);
			String[] messages = new String[2];
			messages[0] = String.format("Der Rang von %s wurde auf %d gesetzt",
					playerName, rank);
			messages[1] = ChatColor.YELLOW + "Der tatsächliche Rang ist "
					+ PlayerManager.getPlayerRanking(player);
