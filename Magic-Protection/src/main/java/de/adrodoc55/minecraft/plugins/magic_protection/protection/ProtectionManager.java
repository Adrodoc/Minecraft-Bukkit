...
public class ProtectionManager {
	...
	public static boolean isBlockTypeProtectable(Material type) {
		return !(MaterialUtils.hasGravity(type)
				|| MaterialUtils.isReplaceable(type)
				|| MaterialUtils.isSpontain(type)
				|| MaterialUtils.isMultiblock(type) || MaterialUtils
					.isDependened(type));
	}
	...
	public static OfflinePlayer getBlockProtector(Block block) {
		...
		ProtectionManager pm = ProtectionManager.getProtectionManager(block
				.getWorld());
		// Kein LoadChunkEvent für SpawnChunks
		pm.loadChunkProtection(block.getChunk());
		if (!isBlockTypeProtectable(block.getType()))
			return null;
		BlockProtectorMetadata blockProtectorMetadata = pm
				.getBlockProtectorMetadata(block);
	...
	private static File getWorldsDir() {
		File worldsDir = new File(MagicProtectionPlugin.instance()
				.getDataFolder(), "worlds");
		worldsDir.mkdirs();
		return worldsDir;
	}
	...
	private ProtectionManager(World world) {
		...
		String message = String
				.format("Neuer ProtectionManager für Welt '%s' mit UUID '%s' initialisiert",
						world.getName(), uuid);
		logger().info(message);
		...
		if (chunkFile.exists()) {
			try {
				...
				while (bb.remaining() >= 18) {
					...
					Block block = ProtectionUtils
							.getBlockOfId(chunk, idOfBlock);
					long mostSigBits = bb.getLong();
					long leastSigBits = bb.getLong();
					UUID id = new UUID(mostSigBits, leastSigBits);
					OfflinePlayer protector = Bukkit.getOfflinePlayer(id);
					if (protector == null) {
						String format = "Spieler mit der UUID '%s' konnte nicht gefunden werden. Lösche Protection vom Block %s";
				...
				String message = String
						.format("IOException beim Laden der Chunkprotection für Chunk: '%s'",
								chunkFile.getName());
	...
	/**
	 * Überschreibt die Chunk Protection für den Chunk nur dann, wenn die Chunk
	 * Protection bereits geladen war und als dirty markiert ist.<br>
	...
	public void saveChunkProtection(Chunk chunk) {
		...
		try {
			...
			for (Block block : blocks) {
				...
				OfflinePlayer protector = getBlockProtectorMetadata(block)
						.value();
			...
			String exMessage = String
					.format("IOException beim Speichern der Protection des Chunks '%s'",
							nameOfChunk);
	...
	private BlockProtectorMetadata getBlockProtectorMetadata(Block block) {
		...
		for (MetadataValue metadata : metadatas) {
			if (JavaPlugin.getPlugin(MagicProtectionPlugin.class).equals(
					metadata.getOwningPlugin())) {
	...
	private void setBlockMetaData(Block block, OfflinePlayer protector) {
		block.setMetadata(
				PROTECTOR,
				new BlockProtectorMetadata(JavaPlugin
						.getPlugin(MagicProtectionPlugin.class), protector));
	...
	private void removeBlockMetaData(Block block) {
		block.removeMetadata(PROTECTOR,
				JavaPlugin.getPlugin(MagicProtectionPlugin.class));
		String chunkName = ProtectionUtils.getNameOfChunk(block.getChunk());
		List<Block> blocks = loadedChunks.get(chunkName);
		if (blocks == null) {
			blocks = new ArrayList<Block>();
