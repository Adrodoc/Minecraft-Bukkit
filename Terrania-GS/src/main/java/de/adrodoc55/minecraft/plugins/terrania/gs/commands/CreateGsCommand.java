...
public class CreateGsCommand extends GsCommand {
	...
	protected boolean execute(CommandContext context) {
		...
		WorldGuardPlugin worldGuardPlugin = JavaPlugin
				.getPlugin(WorldGuardPlugin.class);
		ProtectedRegion region = worldGuardPlugin.getRegionManager(world)
				.getRegion(regionName);
		if (region == null) {
			String message = String
					.format("Es konnte keine Region mit dem Namen '%s' in der Welt %s gefunden werden.",
		...
		Selection selection = JavaPlugin.getPlugin(WorldEditPlugin.class)
				.getSelection(player);
		if (selection == null) {
			String message = "Du musst eine World Edit Selection haben um diesen Befehl aus zu führen.";
			MinecraftUtils.sendError(player, message);
			context.setUsage("");
			return false;
		}
		String missingSignMessage = "Du musst genau ein Schild ausgewählt habem um diesen Befehl auszuführen.";
		...
		String message = String.format(
				"Das Grundstück %s wurde erfolgreich in der Welt %s erstellt.",
	...
	protected List<String> tabComplete(TabCompleteContext context) {
		...
		if (REGION.equals(key)) { // complete Region name
			if (context.getSender() instanceof Player) {
				...
				final List<String> gsNames = CollectionUtils.collect(
						grundstuecke, new Closure<Grundstueck, String>() {
							@Override
							public String call(Grundstueck grundstueck) {
								return grundstueck.getName();
							}
						});
				RegionManager rm = JavaPlugin.getPlugin(WorldGuardPlugin.class)
						.getRegionManager(player.getWorld());
				Set<String> allRegionNames = rm.getRegions().keySet();
				List<String> usableRegionNames = CollectionUtils.findAll(
