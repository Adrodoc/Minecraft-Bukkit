...
public abstract class ConcreteGsCommand extends GsCommand {
	...
	protected final boolean execute(CommandContext context) {
		...
		if (worldName != null) {
			...
			if (world == null) {
				String message = String.format(
						"Die Welt %s konnte nicht gefunden werden.", worldName);
			...
			if (context.getSender() instanceof Player) {
				...
				String message = "Du musst eine Welt angeben um diesen Befehl aus der Konsole benutzen zu können.";
				MinecraftUtils.sendError(context.getSender(), message);
				return false;
			}
		}
		...
		if (gs == null) {
			String message = String
					.format("Das Grundstück %s konnte nicht in der Welt %s gefunden werden.",
							gsName, world.getName());
			MinecraftUtils.sendError(context.getSender(), message);
			context.setUsage("");
			return false;
		}
	...
	protected abstract List<String> tabCompleteAdditionalParams(
