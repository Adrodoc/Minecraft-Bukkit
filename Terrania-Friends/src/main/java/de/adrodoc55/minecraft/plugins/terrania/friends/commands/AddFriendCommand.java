...
public class AddFriendCommand extends FriendsCommand {
  ...
  protected boolean execute(CommandContext context) {
    ...
    String friendFormat =
        ChatColor.GREEN + "[Friends] " + ChatColor.YELLOW + "%s hat dich als Freund hinzugefügt.";
  ...
  protected List<String> tabComplete(TabCompleteContext context) {
    ...
    Player player = (Player) context.getSender();
    ArrayList<? extends Player> onlinePlayers =
        CollectionUtils.newArrayList(Bukkit.getOnlinePlayers());
    onlinePlayers.remove(player);
    List<String> onlinePlayerNames =
        CollectionUtils.collect(onlinePlayers, new Closure<Player, String>() {
          @Override
