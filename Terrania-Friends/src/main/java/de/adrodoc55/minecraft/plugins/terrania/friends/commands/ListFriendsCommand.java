...
public class ListFriendsCommand extends FriendsCommand {
  ...
  protected boolean execute(CommandContext context) {
    ...
    if (playerName != null) {
      ...
      if (context.getSender() instanceof Player) {
        ...
        String message =
            "Du musst einen Spieler angeben um diesen Befehl aus der Konsole benutzen zu können.";
    ...
    sb.append("Freunde:\n");
    if (friends.isEmpty()) {
      sb.append(" " + ChatColor.GOLD + player.getName() + " hat keine Freunde.\n");
    } else {
      List<String> names = CollectionUtils.collect(friends, new Closure<OfflinePlayer, String>() {
        @Override
  ...
  protected List<String> tabComplete(TabCompleteContext context) {
    ArrayList<OfflinePlayer> offlinePlayers =
        CollectionUtils.newArrayList(Bukkit.getOfflinePlayers());
    List<String> playerNames =
        CollectionUtils.collect(offlinePlayers, new Closure<OfflinePlayer, String>() {
          @Override
